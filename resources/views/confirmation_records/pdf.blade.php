...
<head>
    <meta charset="UTF-8">
    <title>Confirmation Records Export</title>
    <style>
...
<body>
    ...
    @if (!empty($activeFilters))
        <div class="filters">
            ...
            @if (!empty($filters['subscriber_imsi']))
                <span>Subscriber IMSI: <em>{{ $filters['subscriber_imsi'] }}</em></span>
            @endif
            @if (!empty($filters['crce_operation']))
                <span>CRCE Operation: <em>{{ $filters['crce_operation'] }}</em></span>
            @endif
            @if (!empty($filters['service_type']))
                <span>Service Type: <em>{{ $filters['service_type'] }}</em></span>
            @endif
        </div>
    @endif
